...
export const VotesProvider = props => {
	...
	const getVotesForDriversComments = useCallback(
		carId => {
			voteService.getVotes(carId).then(setVotes).catch(setErrorMessage);
		},
		[setErrorMessage]
	);
	...
	const getUserVotes = useCallback(() => {
		voteService.getUserVotes().then(setVotes).catch(setErrorMessage);
	}, [setErrorMessage]);
